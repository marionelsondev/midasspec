...
- `midas status` lists every folder under `docs/specs/` with slug, issue counts (total, done, pending), and whether it has been broken down.
...
- At least one spec folder exists under `docs/specs/` (issue 03).

## Main Flow

1. User runs `midas status`.
2. The CLI scans `docs/specs/`, parses each `INDEX.md`, and prints the summary table.
